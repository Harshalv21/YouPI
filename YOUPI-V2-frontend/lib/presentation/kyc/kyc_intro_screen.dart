...
class KycIntroScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Bug #5 fix: no need to create a KycViewModel here -- this screen never
    // reads one, and the KYC ShellRoute in app_router.dart now provides a
    // single shared instance for the whole intro -> aadhaar -> pan flow.
    //
    // BUG FIX (app exits on back press): mpin_setup_screen.dart reaches this
    // screen via context.go('/kyc/intro'), which *replaces* the entire
    // navigator stack -- so this screen sits at the root with nothing left
    // to pop to. Flutter's default behaviour when the root route can't pop
    // is to let the system back button close the Activity, i.e. the app
    // exits. PopScope intercepts that: when there's nothing to pop, treat
    // back the same as tapping "Skip for now" and send the user to the
    // dashboard instead of letting the OS kill the app.
    return PopScope(
      canPop: false,
