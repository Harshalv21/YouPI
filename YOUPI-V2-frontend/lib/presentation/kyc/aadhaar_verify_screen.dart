...
class _AadhaarVerifyScreenState extends State<AadhaarVerifyScreen> {
  // Bug #3 fix: lives on State now, so it survives rebuilds (e.g. the
  // resend-countdown ticking every second no longer wipes what the user typed).
  String _otp = '';
  // Same resend-staleness guard as the main OTP screen (see otp_verify_screen.dart):
  // bump this on resend so a leftover code from before the resend can't be
  // submitted against a newer verification attempt once this is wired to a
  // real Aadhaar OTP backend (currently mocked, so it's inert for now).
  int _resendGeneration = 0;

  @override
  Widget build(BuildContext context) {
    // Bug #5 fix: KycViewModel is now provided once, above the whole /kyc/*
    // flow (see the KYC ShellRoute in app_router.dart), so it's shared across
