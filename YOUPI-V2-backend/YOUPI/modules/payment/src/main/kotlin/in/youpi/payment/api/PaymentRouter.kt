...
class PaymentRouter(private val paymentService: PaymentService) {
    ...
    @RouterOperations(
        ...
        RouterOperation(path = "/webhooks/razorpay", method = [RequestMethod.POST],
            operation = Operation(operationId = "razorpayWebhook", summary = "Razorpay webhook handler",
                description = "Receives Razorpay webhook events. No authentication required. Uses HMAC-SHA256 signature verification (single X-Razorpay-Signature header, no timestamp concat -- unlike the Cashfree scheme this replaces).",
                tags = ["Payment"],
    ...
    // Razorpay sends a SINGLE signature header (X-Razorpay-Signature) --
    // plain HMAC-SHA256 of the raw body. Unlike Cashfree's scheme (two
    // headers: x-webhook-signature + x-webhook-timestamp, signed string =
    // timestamp + rawBody), there's no timestamp to read or concatenate here.
